-- Optional ZenUI Home widget. ZenUI bottom-bar discovery uses OnePlugin:launch().
...
local plugin_dir = integration_dir
    and integration_dir:match("^(.*)/one_reader/integrations$") or nil
local icon_path = plugin_dir and plugin_dir .. "/icons/one-seven-days.svg" or nil

local Integration = {
    name = "zenui",
    item_id = "one.recent_seven_days",
    registered = false,
    plugin = nil,
}
...
function Integration:_livePlugin()
    ...
    if fm and type(fm.one) == "table" then return fm.one end
    return self.plugin
end

function Integration:_openRecentDays()
    local plugin = self:_livePlugin()
    if not plugin or type(plugin.openRecentDays) ~= "function" then return false end
    plugin:openRecentDays()
    return true
end

-- Install a user-visible copy so ZenUI can auto-match it and SimpleUI users can
-- also copy/select the same production SVG without depending on this module.
function Integration:_installIcon()
    ...
    local ok, result = pcall(function()
        ...
        return ffiutil.copyFile(icon_path,
            icons_dir .. "/one-seven-days.svg") == true
    end)
...
function Integration:_buildWidget(ctx)
    ...
    tap.onTapOneRecent = function(tap_self, _arg, ges)
        ...
        return self:_openRecentDays()
    end
    tap.onHoldOneRecent = function(tap_self, _arg, ges)
        if not (tap_self.dimen and ges and ges.pos
                and tap_self.dimen:contains(ges.pos)) then
            return false
        end
...
function Integration:_tryRegister()
    ...
    if self.registered then
        logger.info("[ONE] registered ZenUI recent-seven-days widget")
    end
