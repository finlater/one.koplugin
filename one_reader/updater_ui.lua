-- UI and subprocess orchestration for the updater. Network, verification,
-- extraction, and rollback logic remain isolated in one_reader.updater.
local ConfirmBox = require("ui/widget/confirmbox")
local DownloadDialog = require("one_reader.download_dialog")
...
function UpdaterUI:_show_release(release)
    if self.updater.compare_versions(
        ...
        UIManager:show(InfoMessage:new{
            text = T(_("ONE Plugin is up to date (v%1)."),
...
function UpdaterUI:check(manual)
    ...
    self:_run_subprocess(manual and _("Checking for updates…") or nil, function()
        ...
        if not result or not result.release then
            logger.warn(LOG_MODULE, "check failed:",
...
function UpdaterUI:install(release)
    ...
    local dialog = DownloadDialog:new{
        title = T(_("Preparing ONE Plugin v%1…"), release.version),
        progress_max = 100,
    ...
    self:_run_subprocess(nil, function()
        ...
        if not result or not result.success then
            logger.err(LOG_MODULE, "installation failed:",
                result and result.error or "cancelled")
